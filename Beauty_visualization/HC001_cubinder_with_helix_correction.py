...
# read cubinder --------------------------------------------------------------------------------
events_cubinder = np.genfromtxt("../Outputs/Dataframes/HC001_events_helix_cubinder.csv", delimiter=',')
events_cubinder = pd.DataFrame(data={'event_id':events_cubinder[:,0],
                              ...
                              'event_helix':events_cubinder[:,6]})

fig.add_trace(
    go.Scatter3d(x=events_cubinder["sen_phase"],y=events_cubinder["cos_phase"],z=events_cubinder["event_helix"]),
    row=1, col=1
)

# read cubinder --------------------------------------------------------------------------------
diffs = np.genfromtxt("../Outputs/Dataframes/HC001_helix_matrix_distances.csv", delimiter=',')
