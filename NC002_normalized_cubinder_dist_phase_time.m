% arocha@inaoep
% Este script ejecuta el cubindro normalizado con fase y tiempo, dejando
% fija la freucencia
...
sampling_rate = 100;
seconds= 10;  
min_phase = -pi;  max_phase = pi;  delta_phase = pi/4;  % Phase is naturally in the range (0,1)
freq = 2;  % fijando la frecuencia con valores dentro del rango normalizado
...
for phase = min_phase:delta_phase:max_phase
        ...
        for l = 1:length(S_time)
             namefrequency = string(S_freq(l)/10);
             tname = (S_time(l) - 1) / ( seconds - 1 );
             nametime = string(tname);
             tphase = ((S_phase(l)-pi)-min_phase)/(max_phase-min_phase);
             namephase = string(tphase);
             pphase= (phase-min_phase)/(max_phase-min_phase);
             truephase = string(pphase);
             name = strcat( {'p:'},namephase, {', f:'},namefrequency, {', t:'},nametime, {', phi:'},truephase);
             disp(name);
             labels = [labels,name];
             id = id +1;
             id_s = [id_s,id];
...
csvwrite('Outputs/Dataframes/NC002_events_cubinder.csv',dataframe)
...
csvwrite('Outputs/Dataframes/NC002_matrix_distances_phase_time.csv',senosoidal_matrix_distances)
...
function [total_diff] = parDistancesCubinder(S1_phase,S1_freq,S1_time, S2_phase,S2_freq,S2_time)
   %Compute distances
           ...
           if ((abs(S1_phase -S2_phase))<=0.5)
                  phase_diff = abs(((S1_phase)-(S2_phase)));
           else
                  phase_diff = abs((1)-abs((S1_phase)-(S2_phase)));
           end
